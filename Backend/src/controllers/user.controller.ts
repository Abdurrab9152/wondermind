...
import { accessTokenCookieOptions } from '../utils/index.js'
...
const generateAccessToken = async(userId: mongoose.Schema.Types.ObjectId) => {
    try {

        const user = await User.findById(userId)
        const accessToken = user.generateAccessToken()
        // const refreshToken =  user.generateRefreshToken() Maybe implement this in future if needed 
...
const getAllUsers = asyncHandler(async(req: Request, res: Response) => {
    ...
    return res.status(200).json(new ApiResponse(200, users, "User found successfully."))
})
